...
__all__ = ['get_user_by_username', 'get_friend_usernames',
    ...
    'NotFound', 'InvalidDictionary', 'PUBLIC_USERLINE_KEY','save_user_status','get_user_status']
...
#APOOL = ConnectionPool('Activibe')

#USERINFO = ColumnFamily(APOOL, 'UserInfo')
#DOCTORINFO = ColumnFamily(APOOL, 'DoctorInfo')
#DOCTORFOLLOWINGUSER = ColumnFamily(APOOL, 'DoctorFollowingUser')
#USERSTATUS = ColumnFamily(APOOL, 'UserStatus')
#USERSERVEDBYDOCTOR = ColumnFamily(APOOL, 'UserServedByDoctors')
...
#Activibe_Insert APIs

def save_user_status(user,status):
    """
    Saves user updates
    """
    USERINFO.insert(str(username),status)

#Activibe_Retrive APIs
...
def get_user_status(user):
    ...
    start=None
    limit=40
    return _get_line(USERINFO,str(username),start,limit)
